# dotfiles
My dots

## Installation
These are detailed instructions for installing my dotfiles on Ubuntu; the installation will vary on other non-Debian based Linux distributions.

1. Install [Homebrew](https://brew.sh/).

2. Install the following packages:
```bash
brew install fzf
brew install neovim
```

3. Install the kickstart.nvim external dependencies:
```bash
sudo apt-get install git make unzip build-essential xclip ripgrep
```

4. Install a [Nerd Font](https://www.nerdfonts.com/). `kickstart.nvim` does not require you to install a Nerd Font but this configuration assumes you have one installed by default.

5. Run the convenience script to install all config files/directories as symlinks.
```bash
./install.sh
```

## Recommendations

Some recommended tools:
* [Kitty](https://github.com/kovidgoyal/kitty): Terminal emulator
* [tmux](https://github.com/tmux/tmux): Terminal multiplexer
* [OpenCode](https://github.com/tmux/tmux): AI coding agent CLI tool
* [LazyGit](https://github.com/jesseduffield/lazygit): Git TUI tool

